...
public class DownloadImageService extends IntentService {
    ...
    public static Intent makeIntent(Context context,
        ...
        Intent myIntent = new Intent(context, DownloadImageService.class);
        myIntent.setData(url);
        myIntent.putExtra(REQUEST_CODE, requestCode);
        Messenger messenger = new Messenger(downloadHandler);
        myIntent.putExtra(MESSENGER, messenger);
        myIntent.putExtra(DIRECTORY_PATHNAME, directoryPathname);
    ...
    public void onHandleIntent(Intent intent) {
        ...
        Uri downloadUrl = intent.getData();
        ...
        String dirPathName = intent.getExtras().getString(DIRECTORY_PATHNAME);

        Log.d("directory", dirPathName);

        // Download the requested image.
        // @@ TODO -- you fill in here.


        Uri downloadedFilePath = Utils.downloadImage(getApplicationContext(), downloadUrl, dirPathName);

        Log.d("file", downloadedFilePath.toString());

        // Extract the Messenger stored as an extra in the
        // intent under the key MESSENGER.
        // @@ TODO -- you fill in here.

        Messenger messenger = (Messenger) intent.getExtras().getParcelable(MESSENGER);

        // Send the path to the image file back to the
        // MainActivity via the messenger.
        // @@ TODO -- you fill in here.

        sendPath(messenger, downloadedFilePath,downloadUrl);
